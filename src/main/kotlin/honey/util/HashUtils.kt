...
object HashUtils {
    ...
    fun md5(text: String): String {
        return String(encodeHex(md5Bytes(text.toByteArray(Charsets.UTF_8))))
    }
    ...
    fun sha1(text: String): String {
        return String(encodeHex(sha1Bytes(text.toByteArray(Charsets.UTF_8))))
    }

    fun sha1(file: File): String {
        return String(encodeHex(digestFile(file, SHA_1)))
    }

    fun digestFile(file: File, method: String): ByteArray {
        val digest = getDigest(method)
    ...
    fun sha256(text: String): String {
        return String(encodeHex(sha256Bytes(text.toByteArray(Charsets.UTF_8))))
    }
